...
with Timer(name='Preprocess', text=timer_text):
    vocab = run(vocab, vocabulary_pipeline)
    group_1 = run(group_1, text_analysis_pipeline)
...
with Timer(name='Locate terms', text=timer_text):
    ...
    terms_locs = find_term_locations_in_docs(terms, group_1[COL_LEMMA])
...
with Timer(name='TFIDF', text=timer_text):
    vocab[TFIDF] = tfidfs(term_locs=terms_locs, docs=group_1[COL_LEMMA])

with Timer(name='Trees', text=timer_text):
    trees = tree.make_trees_for_occurrences_v2(
        term_locs=terms_locs, stanza_docs=group_1[COL_STANZA_DOC]
    )
...
contexts_by_term = context.collect_context_words_multiple(
    terms_locs, group_1[COL_LEMMA], window=3
)
...
ctx_terms_locs = find_term_locations_in_docs(ctx_terms, group_1[COL_LEMMA])
