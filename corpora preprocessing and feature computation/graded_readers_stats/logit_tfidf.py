...
def logit_tfidf(X, y):
    """
    Trains Logistic Regression using TF-IDF and evaluates the model.

    Parameters
    ----------
    X : pandas.core.series.Series
        Pandas Series containing strings. Each string represents a document.

    y : pandas.core.series.Series
        Pandas Series containing strings. Each string represents a label.
        Example labels:
            Inicial
            Intermedio
            Avanzado
    ...
    y_train_pred = pd.DataFrame(lr.predict(X_train), index=X_train_index, columns=['lr_bow_pred'])
    y_test_pred = pd.DataFrame(lr.predict(X_test), index=X_test_index, columns=['lr_bow_pred'])
...
def print_report(y_train_true, y_train_pred,
...
def evaluate(y_train_true, y_train_pred,
             y_test_true, y_test_pred,
             column):
    labels = [
        'accuracy',
